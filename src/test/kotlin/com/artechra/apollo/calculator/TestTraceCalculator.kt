...
class TestTraceCalculator {
    ...
    fun trivialTraceShouldReturnRootUsage() {
        ...
        assertEquals("Wrong CPU total", 1, resUsage.totalCpuMsec)
    ...
    fun trivialTraceShouldReturnRootSpanEstimate() {
        ...
        val testResourceUsageManager = StubResourceUsageManager(hashMapOf(containerId to ResourceUsage(12, 2, 3, 4)),
                hashMapOf(networkAddr to 123L))
        val trivialTrace = Trace(setOf(Span( "54C92796854B15C8","54C92796854B15C8", "192.168.1.2",baseTime+100000, baseTime+110000)))
        val tc = TraceCalculator(Trace(setOf<Span>(Span("AA", "AA", "1.1.1.1", 1515237362000, 1515237362000))), testResourceUsageManager, testNetInfo)
        val estimate = tc.calculateEnergyEstimateForTrace(trivialTrace) ;
